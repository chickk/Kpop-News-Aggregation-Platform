...
from app.interfaces.repositories import IArtistRepository, IArticleRepository, IGroupRepository, IEventRepository, ISourceRepository
...
class MongoArticleRepository(BeanieRepository[Article_db], IArticleRepository):
    ...
    async def get_by_artist(
        ...
        if not pyd_id: return []
            
        query = self.model.find(
            {"artists_mentioned_ids": pyd_id},
            session=self.session
        )
        return await query.skip(skip).limit(limit).to_list()
